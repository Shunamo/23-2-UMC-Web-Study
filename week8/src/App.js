import axios from 'axios';//서버랑 통신할때 
...
function App() {
  ...
  const API_KEY = "736168f28063eff0ff57249c4ae8cdac"; // 각자 개인의 API KEY를 발급받아 사용해주세요. 
  ...
  const searchWeather = async (e) => {
    if(e.key === 'Enter') {
      try {
        const data= await axios.get(url); //await: get하기 전까지 잠시 실행 중단 get의 인자: url로 받기
        setResult(data);
        console.log(data);
        if(data.status===200){
          alert('success');
